...
class ONNXModel(BaseModel):
    ...
    def from_path(
        ...
        if ort is None:
            raise ModelLoadError(
                "onnxruntime is required to load ONNX models. Install `onnxruntime`."
            )
        ...
        try:
            ...
            raise ModelLoadError(
                f"Failed to initialize ONNX runtime for {path}"
            ) from exc
    ...
    def predict(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(payload, Mapping):
            raise ModelPredictError(
                "ONNXModel expects a mapping of input tensor names."
            )
        ...
        for idx, name in enumerate(self._input_names):
            ...
            try:
                ...
                expected_rank = (
                    len(expected_shape) if expected_shape is not None else arr.ndim
                )
            except Exception:
                expected_shape = None
            ...
            if arr.ndim == 1 and expected_rank > 1:
                ...
                if (
                    expected_rank == 2
                    and isinstance(expected_shape, (list, tuple))
    ...
    def validate_input(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if missing:
            raise ModelPredictError(
                f"Missing required ONNX input(s): {', '.join(missing)}"
            )
