...
class SklearnModel(BaseModel):
        ...
        **_: Any,
        ...
        try:
            ...
            raise ModelLoadError(
                f"Failed to load sklearn artifact from {path}"
            ) from exc
    ...
    def predict(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        if "inputs" not in input_data:
            raise ModelPredictError(
                "SklearnModel expects an 'inputs' key with iterable features."
            )
        ...
        if input_data.get("return_proba") and hasattr(self._estimator, "predict_proba"):
            try:
                ...
                raise ModelPredictError(
                    "SklearnModel probability prediction failed"
                ) from exc
    ...
    def validate_input(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "inputs" not in payload:
            raise ModelPredictError(
                "SklearnModel expects an 'input' or 'inputs' field."
            )
