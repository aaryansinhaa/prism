...
class TestUploadAndRunWorkflow:
    ...
    def test_upload_includes_prediction_url(self):
        ...
        assert (
            "Prediction URL" in response.text
...
class TestPredictionWorkflow:
    ...
    def test_prediction_rejects_invalid_json(self):
        ...
        response = client.post(
            "/predict-result",
            data={"model_id": "test-model", "input_data": "not valid json {"},
        )
    ...
    def test_prediction_rejects_payload_not_matching_expected_contract(
    ...
    def test_predict_page_shows_contract_hints_and_sample_payload(
        self, monkeypatch, tmp_path
    ):
...
class TestTunnelIntegration:
    ...
    def test_success_response_without_tunnel(self):
        ...
        html = upload_success_response(
            model_id="test-model", port=8001, tunnel_url=None
        )
    ...
    def test_success_response_with_tunnel(self):
        ...
        html = upload_success_response(
            model_id="test-model", port=8001, tunnel_url="https://test-model.ngrok.io"
        )
...
class TestErrorHandling:
    ...
    def test_prediction_error_component_includes_navigation(self):
        ...
        html = prediction_error_component(
            error="Test error message", model_id="test-model"
        )

        assert "alert-error" in html
        assert "Test error message" in html
        assert "Try Again" in html
        assert "Upload New Model" not in html
...
class TestHTMXAttributes:
    ...
    def test_loading_indicator_has_htmx_class(self):
        ...
        response = client.get("/upload-model")
        assert (
            'class="htmx-indicator' in response.text
            or "htmx-indicator" in response.text
        )
...
class TestAccessibility:
    ...
    def test_error_messages_are_readable(self):
        ...
        response = client.post(
            "/predict-result", data={"model_id": "test", "input_data": "invalid {{"}
        )
