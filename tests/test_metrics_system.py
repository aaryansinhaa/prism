...
class TestMetricsCollector:
    ...
    def test_aggregated_metrics_has_all_fields(self):
        ...
        collector.record_request(
            latency_ms=100.0, success=True, cpu_percent=25.5, memory_mb=512.0
        )
...
class TestModelMetricsEndpoint:
    ...
    def test_metrics_values_are_numeric(self):
        ...
        assert all(isinstance(req, (int, float)) for req in data["requests"])
        assert all(isinstance(latency, (int, float)) for latency in data["latency"])
        assert all(
            isinstance(throughput, (int, float)) for throughput in data["throughput"]
        )
...
class TestModelControlCenterUI:
    ...
    def test_control_center_has_metrics_forms(self):
        ...
        assert (
            'id="metricsConfigForm"' in response.text
            or "metricsConfigForm" in response.text
        )
    ...
    def test_control_center_form_submission(self):
        ...
        assert (
            'hx-post="/api/metrics-config"' in response.text
            or "/api/metrics-config" in response.text
        )
