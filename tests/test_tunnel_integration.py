...
def test_upload_and_run_without_tunnel(monkeypatch, tmp_path):
    ...
    with TestClient(app) as client:
        response = client.post(
            ...
            files={"file": ("model.onnx", io.BytesIO(b"model-bytes"), "application/octet-stream")},
        )
...
def test_upload_and_run_with_tunnel_enabled(monkeypatch, tmp_path):
    ...
    import app.routing.models as models_module
...
def test_registry_contains_tunnel_info(monkeypatch, tmp_path):
    ...
    assert data["models"]["test_model_001"]["tunnel_url"] == "https://testmodel001.ngrok.io"
